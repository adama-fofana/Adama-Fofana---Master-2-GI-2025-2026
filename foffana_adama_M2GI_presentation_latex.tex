...
\begin{titlepage}
    ...
    \framebox(150,150){\parbox{150pt}{\centering\vspace{65pt}LOGO\\Universit\\Nangui Abrogoua}}
    ...
    {\large \textbf{Encadré par :} Pr. Konan Kouassi} \\
...
class IntegrationNumerique:
    ...
    def methode_trapezes(self, f, a, b, n):
...
def tester_integration():
    integrateur = IntegrationNumerique()
    
    # Test avec la fonction de Runge
    a, b = -1, 1
    ...
    for n in n_values:
        # Valeur exacte (approximative)
        exact = 2/26 * np.arctan(5)  # Pour f_runge
        
        # Calcul avec différentes méthodes
        gl = integrateur.gauss_legendre(f_runge, a, b, n)
        simp = integrateur.simpson_composite(f_runge, a, b, n)
        trap = integrateur.methode_trapezes(f_runge, a, b, n)
        
        # Erreurs
        resultats[n] = {
            'gauss_legendre': {'valeur': gl, 'erreur': abs(gl - exact)},
            'simpson': {'valeur': simp, 'erreur': abs(simp - exact)},
            'trapezes': {'valeur': trap, 'erreur': abs(trap - exact)}
        }
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    resultats = tester_integration()
    
    # Affichage des résultats
    for n, methods in resultats.items():
        print(f"\nNombre de points: n = {n}")
        for methode, data in methods.items():
            print(f"  {methode:15s}: {data['valeur']:.10f} (erreur: {data['erreur']:.2e})")
\end{lstlisting}
...
\bibitem{quarteroni}
Quarteroni, A., Saleri, F., \& Sacco, R. (2007). \emph{Méthodes numériques pour le calcul scientifique}. Springer.

\bibitem{heath}
Heath, M. T. (2018). \emph{Scientific Computing: An Introductory Survey}. SIAM.

\bibitem{stroud}
Stroud, A. H. (1971). \emph{Approximate Calculation of Multiple Integrals}. Prentice-Hall.

\bibitem{press}
Press, W. H., Teukolsky, S. A., Vetterling, W. T., \& Flannery, B. P. (2007). \emph{Numerical Recipes: The Art of Scientific Computing}. Cambridge University Press.

\bibitem{suli}
Süli, E., \& Mayers, D. F. (2003). \emph{An Introduction to Numerical Analysis}. Cambridge University Press.
\end{thebibliography}
